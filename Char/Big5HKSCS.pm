...
sub Char::Big5HKSCS::escape_script {
    ...
    if (m/^ use Char::Ebig5hkscs(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        if (s/^ \s* use \s+ Char::Big5HKSCS \s* ([^;]*) ; \s* \n? $//oxms) {
...
sub e_use_noimport {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (CORE::open($fh, $realfilename)) {
            ...
            if ($script =~ m/^ \s* use \s+ Char::Big5HKSCS \s* ([^;]*) ; \s* \n? $/oxms) {
...
and the cellular phone. This software treats Big5-HKSCS directly. Therefor there is
not UTF8 flag.
...
=head1 Perl5.6 Emulation On perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Ebig5hkscs.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Ebig5hkscs::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Ebig5hkscs::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005 other than MacPerl, Char::Big5HKSCS software emulate perl5.6's
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  If you are using perl5.005, Char::Big5HKSCS software emulate perl5.6's open function.
  ...
  open(FH, "<:crlf:encoding(Char::Big5HKSCS)", $path) or die "can't open $path: $!";
