...
written by Big5-HKSCS.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. But this "text" means US-ASCII text, and popular Big5-HKSCS
...
Perl3 or later can treat Big5-HKSCS.

Perl is designed to make the easy jobs easy, without making the hard jobs
impossible. Char::Big5HKSCS software is Perl program designed to make the "easy jobs easy".
...
=item * Handles Raw Big5-HKSCS, Doesn't use UTF8 flag like JPerl
...
Definitions in Char/Ebig5hkscs.pm.
  ...
  ${Char::Ebig5hkscs::not_space}      qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x09\x0A\x0B\x0C\x0D\x20])}
...
On perl5.008001 or later, perl5.010, perl5.012, perl5.014, perl5.016, if path is
ended by chr(0x5C), chdir succeeds when a short path name (8dot3name) can be
acquired according to COMMAND.COM or cmd.exe. However, leaf-subdirectory of the
current directory is a short path name (8dot3name).
...
It has already been achieved by Big5-HKSCS designed for combining with
...
Contrasting is Encode module and describing "use Char::Big5HKSCS;" on this software,
