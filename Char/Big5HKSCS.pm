...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Ebig5hkscs::uc($char[$i]) ne Char::Ebig5hkscs::lc($char[$i]))) {
            $char[$i] = '[' . Char::Ebig5hkscs::uc($char[$i]) . Char::Ebig5hkscs::lc($char[$i]) . ']';
        }
...
sub e_require {
    ...
    return qq<Char::Ebig5hkscs::require '$expr';>;
}
...
http://search.cpan.org/dist/Char-Char::Big5HKSCS/Char::Big5HKSCS/JA.pm
...
Definitions in Char/Ebig5hkscs.pm.
  ...
  @{Char::Ebig5hkscs::eV}             qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x0C\x0A\x0D])}
...
=item * Order Of Character
  ...
  This function returns the numeric value ASCII or Big5-HKSCS of the first character
...
=item * Length By Big5-HKSCS Character
  ...
  $string is omitted, it returns the Char::Big5HKSCS::length of $_.
...
=item * Substr By Big5-HKSCS Character
  ...
  area past the end of the string, Char::Big5HKSCS::substr raises an exception.
...
=item * Rindex By Big5-HKSCS Character
  ...
  $rindex = Char::Big5HKSCS::rindex($string,$substring,$position);
  $rindex = Char::Big5HKSCS::rindex($string,$substring);

  This function works just like Char::Big5HKSCS::index except that it returns the position of
