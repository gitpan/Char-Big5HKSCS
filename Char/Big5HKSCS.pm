package Char::Big5HKSCS;
######################################################################
#
# Char::Big5HKSCS - Source code filter to escape Big5-HKSCS script
#
# http://search.cpan.org/dist/Char-Char::Big5HKSCS/
#
...
BEGIN { CORE::require Char::Ebig5hkscs; }
...
sub unimport {}
sub Char::Big5HKSCS::escape_script;
...
my $function_ord;         # ord()   to ord()   or Char::Big5HKSCS::ord()
my $function_ord_;        # ord     to ord     or Char::Big5HKSCS::ord_
my $function_reverse;     # reverse to reverse or Char::Big5HKSCS::reverse
my $function_getc;        # getc    to getc    or Char::Big5HKSCS::getc
...
END
    ...
    print Char::Big5HKSCS::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Char::Ebig5hkscs::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            Char::Ebig5hkscs::unlink "$filename.e";
        }
        elsif (Char::Ebig5hkscs::z("$filename.e")) {
            Char::Ebig5hkscs::unlink "$filename.e";
        }
        else {
            my $e_mtime   = (Char::Ebig5hkscs::stat("$filename.e"))[9];
            my $mtime     = (Char::Ebig5hkscs::stat($filename))[9];
            my $__mtime__ = (Char::Ebig5hkscs::stat(__FILE__))[9];
    ...
    if (not Char::Ebig5hkscs::e("$filename.e")) {
        my $fh = gensym();

        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
        }
        else {
            Char::Ebig5hkscs::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Big5HKSCS::escape_script($filename);
        print {$fh} $e_script;

        my $mode = (Char::Ebig5hkscs::stat($filename))[2] & 0777;
    ...
    my $fh = gensym();
    Char::Ebig5hkscs::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Big5HKSCS::escape_script {
    ...
    my $fh = gensym();
    Char::Ebig5hkscs::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ebig5hkscs(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ebig5hkscs %s;\n", $Char::Big5HKSCS::VERSION); # require run-time routines version

        # use Char::Big5HKSCS version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Big5HKSCS \s* ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Big5HKSCS::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Big5HKSCS.pm}} @INC;
...
****************************************************
              ...
              CONFLICT Char/Big5HKSCS.pm FILE
...
END
                    ...
                    die "Script $0 expects Char/Big5HKSCS.pm $version, but @{[__FILE__]} is version $Char::Big5HKSCS::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ebig5hkscs.pm $version, but \\\$Char::Ebig5hkscs::VERSION is \$Char::Ebig5hkscs::VERSION" if \$Char::Ebig5hkscs::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Big5HKSCS::VERSION) {
                    die "Script $0 required Char/Big5HKSCS.pm $version, but @{[__FILE__]} is only version $Char::Big5HKSCS::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Big5HKSCS::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Big5HKSCS::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Big5HKSCS::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Big5HKSCS::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# escape Big5-HKSCS part of script
sub escape {
...
# functions of package Char::Ebig5hkscs
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Char::Ebig5hkscs::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Char::Ebig5hkscs::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Char::Ebig5hkscs::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Char::Ebig5hkscs::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Char::Ebig5hkscs::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Char::Ebig5hkscs::$1(" . e_q ('q', $2,$4,$3) . ")"; }
...
# functions of package Char::Ebig5hkscs
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Char::Ebig5hkscs::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Char::Ebig5hkscs::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Char::Ebig5hkscs::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Char::Ebig5hkscs::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Char::Ebig5hkscs::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Char::Ebig5hkscs::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Big5HKSCS::re_r=eval $Char::Big5HKSCS::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Big5HKSCS::re_t=%s; while($Char::Big5HKSCS::re_t =~ %s){%s local $^W=0; local $Char::Big5HKSCS::re_r=%s; %s$Char::Big5HKSCS::re_t="$`$Char::Big5HKSCS::re_r$'"; pos($Char::Big5HKSCS::re_t)=length "$`$Char::Big5HKSCS::re_r"; } return $Char::Big5HKSCS::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Big5HKSCS::re_r=%s; %s"%s$Char::Big5HKSCS::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::Big5HKSCS::re_n=0; local $Char::Big5HKSCS::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::Big5HKSCS::re_r=%s; %s%s="$Char::Big5HKSCS::re_a${1}$Char::Big5HKSCS::re_r$'"; pos(%s)=length "$Char::Big5HKSCS::re_a${1}$Char::Big5HKSCS::re_r"; $Char::Big5HKSCS::re_a=substr(%s,0,pos(%s)); $Char::Big5HKSCS::re_n++} return %s$Char::Big5HKSCS::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Big5HKSCS::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Big5HKSCS::re_r=%s; %s%s="$`$Char::Big5HKSCS::re_r$'"; pos(%s)=length "$`$Char::Big5HKSCS::re_r"; $Char::Big5HKSCS::re_n++} return %s$Char::Big5HKSCS::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Big5HKSCS::re_r=%s; %s%s="%s$Char::Big5HKSCS::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Big5HKSCS::re_r=%s; %s%s="%s$Char::Big5HKSCS::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Big5HKSCS::re_r=eval $Char::Big5HKSCS::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Big5HKSCS.pm and Char/Ebig5hkscs.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Big5HKSCS;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Big5HKSCS;
  use Char::Big5HKSCS ver.sion;             --- require minimum version
  use Char::Big5HKSCS ver.sion.0;           --- expects version (match or die)
  use Char::Big5HKSCS qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::Big5HKSCS ver.sion qw(ord reverse getc);
  use Char::Big5HKSCS ver.sion.0 qw(ord reverse getc);

  # "no Char::Big5HKSCS;" not supported

  or

  $ perl Char/Big5HKSCS.pm Big5-HKSCS_script.pl > Escaped_script.pl.e
  ...
  functions:
    Char::Big5HKSCS::ord(...);
    Char::Big5HKSCS::reverse(...);
    Char::Big5HKSCS::getc(...);
    Char::Big5HKSCS::length(...);
    Char::Big5HKSCS::substr(...);
    Char::Big5HKSCS::index(...);
    Char::Big5HKSCS::rindex(...);
    <*>
    glob(...);
    CORE::chop(...);
    CORE::ord(...);
...
Char::Big5HKSCS software is "middleware" between perl interpreter and your Perl script
...
By "use Char::Big5HKSCS;", it automatically interpret your script as Big5-HKSCS. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Big5HKSCS.pm, Char/Ebig5hkscs.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ebig5hkscs.pm
works well.

   in Char/Ebig5hkscs.pm
   #
...
=head1 Software Composition
   ...
   Char/Big5HKSCS.pm               --- source code filter to escape Big5-HKSCS
   Char/Ebig5hkscs.pm              --- run-time routines for Char/Big5HKSCS.pm
...
You need write 'use Char::Big5HKSCS;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Big5HKSCS;
  ---------------------------------

=head1 Escaping Multiple-Octet Code (Char/Big5HKSCS.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/Big5HKSCS.pm provides)

Char/Big5HKSCS.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Ebig5hkscs::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (Char/Big5HKSCS.pm provides)

Char/Big5HKSCS.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char/Big5HKSCS.pm provides)

Char/Big5HKSCS.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Ebig5hkscs::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Ebig5hkscs::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Ebig5hkscs::ignorecase()' (Char/Big5HKSCS.pm provides)

Char/Big5HKSCS.pm applies calling 'Char::Ebig5hkscs::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ebig5hkscs::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Ebig5hkscs::matched/
  m/regexp/               m/ditto$Char::Ebig5hkscs::matched/
  ?regexp?                m?ditto$Char::Ebig5hkscs::matched?
  m?regexp?               m?ditto$Char::Ebig5hkscs::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Ebig5hkscs::matched/) ?
  s/regexp/replacement/   eval{ Char::Ebig5hkscs::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Ebig5hkscs::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Ebig5hkscs::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Ebig5hkscs::split(qr/regexp/)
  split(m/regexp/)        Char::Ebig5hkscs::split(qr/regexp/)
  split(qr/regexp/)       Char::Ebig5hkscs::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Ebig5hkscs::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Ebig5hkscs::matched/
  m/regexp/b              m/(?:regexp)$Char::Ebig5hkscs::matched/
  ?regexp?b               m?regexp$Char::Ebig5hkscs::matched?
  m?regexp?b              m?regexp$Char::Ebig5hkscs::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Ebig5hkscs::matched/) ?
  s/regexp/replacement/b  eval{ Char::Ebig5hkscs::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Ebig5hkscs::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ebig5hkscs.pm provides)
...
The character classes are redefined as follows to backward compatibility.
  ...
  ---------------------------------------------------------------
   .            ${Char::Ebig5hkscs::dot}
                ${Char::Ebig5hkscs::dot_s}    (/s modifier)
  \d            [0-9]
  \s            \s
  \w            [0-9A-Z_a-z]
...
Also POSIX-style character classes.
  ...
  [:^lower:]    ${Char::Ebig5hkscs::not_lower}
                ${Char::Ebig5hkscs::not_lower_i}    (/i modifier)
  ...
  [:^upper:]    ${Char::Ebig5hkscs::not_upper}
                ${Char::Ebig5hkscs::not_upper_i}    (/i modifier)
...
Definitions in Char/Ebig5hkscs.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Char::Ebig5hkscs::anchor}         qr{\G(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE])*?}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Big5HKSCS.pm provides)

Char/Big5HKSCS.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Big5HKSCS.pm and Char/Ebig5hkscs.pm provide)

Insert 'Char::Ebig5hkscs::' at head of function name. Char/Ebig5hkscs.pm provides your script Char::Ebig5hkscs::*
functions.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Ebig5hkscs::split     Character
  tr///       Char::Ebig5hkscs::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Ebig5hkscs::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Ebig5hkscs::chop      Character
  index       Char::Ebig5hkscs::index     Character
  rindex      Char::Ebig5hkscs::rindex    Character
  lc          Char::Ebig5hkscs::lc        Character
  lcfirst     Char::Ebig5hkscs::lcfirst   Character
  uc          Char::Ebig5hkscs::uc        Character
  ucfirst     Char::Ebig5hkscs::ucfirst   Character
  fc          Char::Ebig5hkscs::fc        Character
  chr         Char::Ebig5hkscs::chr       Character
  glob        Char::Ebig5hkscs::glob      Character
  lstat       Char::Ebig5hkscs::lstat     Character
  opendir     Char::Ebig5hkscs::opendir   Character
  stat        Char::Ebig5hkscs::stat      Character
  unlink      Char::Ebig5hkscs::unlink    Character
  chdir       Char::Ebig5hkscs::chdir     Character
  do          Char::Ebig5hkscs::do        Character
  require     Char::Ebig5hkscs::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Char::Ebig5hkscs::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Char::Ebig5hkscs::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Char::Ebig5hkscs::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Char::Ebig5hkscs::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Char::Ebig5hkscs::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Char::Ebig5hkscs::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Char/Big5HKSCS.pm and Char/Ebig5hkscs.pm provide)

Insert 'Char::Ebig5hkscs::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Char::Ebig5hkscs::r   File or directory is readable by this (effective) user or group
  -w       Char::Ebig5hkscs::w   File or directory is writable by this (effective) user or group
  -e       Char::Ebig5hkscs::e   File or directory name exists
  -x       Char::Ebig5hkscs::x   File or directory is executable by this (effective) user or group
  -z       Char::Ebig5hkscs::z   File exists and has zero size (always false for directories)
  -f       Char::Ebig5hkscs::f   Entry is a plain file
  -d       Char::Ebig5hkscs::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Ebig5hkscs::T   File looks like a "text" file
  -B       Char::Ebig5hkscs::B   File looks like a "binary" file
  -M       Char::Ebig5hkscs::M   Modification age (measured in days)
  -A       Char::Ebig5hkscs::A   Access age (measured in days)
  -C       Char::Ebig5hkscs::C   Inode-modification age (measured in days)
  -s       Char::Ebig5hkscs::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Ebig5hkscs::R   File or directory is readable by this real user or group
  -W       Char::Ebig5hkscs::W   File or directory is writable by this real user or group
  -X       Char::Ebig5hkscs::X   File or directory is executable by this real user or group
  -l       Char::Ebig5hkscs::l   Entry is a symbolic link
  -S       Char::Ebig5hkscs::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Ebig5hkscs::o   File or directory is owned by this (effective) user
  -O       Char::Ebig5hkscs::O   File or directory is owned by this real user
  -p       Char::Ebig5hkscs::p   Entry is a named pipe (a "fifo")
  -b       Char::Ebig5hkscs::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Ebig5hkscs::c   Entry is a character-special file (like an I/O device)
  -u       Char::Ebig5hkscs::u   File or directory is setuid
  -g       Char::Ebig5hkscs::g   File or directory is setgid
  -k       Char::Ebig5hkscs::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Char::Big5HKSCS::' at head of function name when you want character-
oriented function. See 'Character-Oriented Functions'.

  --------------------------------------------------------
  Function   Character-Oriented   Description
  --------------------------------------------------------
  ord        Char::Big5HKSCS::ord
  reverse    Char::Big5HKSCS::reverse
  getc       Char::Big5HKSCS::getc
  length     Char::Big5HKSCS::length
  substr     Char::Big5HKSCS::substr
  index      Char::Big5HKSCS::index          See 'About Indexes'
  rindex     Char::Big5HKSCS::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Ebig5hkscs::index)
  Char::Big5HKSCS::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Ebig5hkscs::rindex)
  Char::Big5HKSCS::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = Char::Big5HKSCS::ord($string);

  This function returns the numeric value (ASCII or Big5-HKSCS character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use Char::Big5HKSCS qw(ord);", ord of your script will be rewritten in
  Char::Big5HKSCS::ord. Char::Big5HKSCS::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::Big5HKSCS::reverse(@list);
  $reverse = Char::Big5HKSCS::reverse(@list);
  ...
  If you import reverse "use Char::Big5HKSCS qw(reverse);", reverse of your script will be
  rewritten in Char::Big5HKSCS::reverse. Char::Big5HKSCS::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::Big5HKSCS::getc(FILEHANDLE);
  $getc = Char::Big5HKSCS::getc($filehandle);
  $getc = Char::Big5HKSCS::getc;
  ...
  If you import getc "use Char::Big5HKSCS qw(getc);", getc of your script will be rewritten in
  Char::Big5HKSCS::getc. Char::Big5HKSCS::getc is not compatible with getc of JPerl.

=item * Length by Big5-HKSCS Character

  $length = Char::Big5HKSCS::length($string);
  $length = Char::Big5HKSCS::length();

  This function returns the length in characters (programmer-visible characters) of
  the scalar value $string. If $string is omitted, it returns the Char::Big5HKSCS::length of
  $_.

  Do not try to use Char::Big5HKSCS::length to find the size of an array or hash. Use scalar
...
=item * Substr by Big5-HKSCS Character
  ...
  $substr = Char::Big5HKSCS::substr($string,$offset,$length,$replacement);
  $substr = Char::Big5HKSCS::substr($string,$offset,$length);
  $substr = Char::Big5HKSCS::substr($string,$offset);
  ...
  my $color  = Char::Big5HKSCS::substr $s, 4, 5;      # black
  my $middle = Char::Big5HKSCS::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::Big5HKSCS::substr $s, 14;        # climbed the green tree
  my $tail   = Char::Big5HKSCS::substr $s, -4;        # tree
  my $z      = Char::Big5HKSCS::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::Big5HKSCS::substr() function as an
  ...
  end of the string, Char::Big5HKSCS::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::Big5HKSCS::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::Big5HKSCS::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::Big5HKSCS::substr $name, 7;      # returns undef, with warning
  Char::Big5HKSCS::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::Big5HKSCS::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::Big5HKSCS::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::Big5HKSCS::substr() acts
  ...
  $x = '1234';
  for (Char::Big5HKSCS::substr($x,1,2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4
      $_ = 'xyz'; print $x,"\n";    # prints 1xyz4
      $x = '56789';
      $_ = 'pq';  print $x,"\n";    # prints 5pq9
  ...
  $x = '1234';
  for (Char::Big5HKSCS::substr($x, -3, 2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4, as above
      $x = 'abcdefg';
      print $_,"\n";                # prints f
  }
...
=item * Index by Big5-HKSCS Character
  ...
  $index = Char::Big5HKSCS::index($string,$substring,$offset);
  $index = Char::Big5HKSCS::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::Big5HKSCS::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by Big5-HKSCS Character
  ...
  $rindex = Char::Big5HKSCS::rindex($string,$substring,$offset);
  $rindex = Char::Big5HKSCS::rindex($string,$substring);

  This function works just like Char::Big5HKSCS::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::Big5HKSCS::index).
  ...
  $pos = Char::Big5HKSCS::length($string);
  while (($pos = Char::Big5HKSCS::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Ebig5hkscs::glob() function. See Char::Ebig5hkscs::glob of
  Char/Ebig5hkscs.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::Big5HKSCS qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::Big5HKSCS qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Functions (Char/Big5HKSCS.pm provides)

Char/Big5HKSCS.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Ebig5hkscs.pm provides)

Char/Ebig5hkscs.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Big5HKSCS' }" at head.
Store the standard module modified for Char::Big5HKSCS software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Big5HKSCS and change
'use utf8;' to 'use Char::Big5HKSCS;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ebig5hkscs.pm provides these
  functions.
...
=item * Char::Big5HKSCS::substr as Lvalue

Char::Big5HKSCS::substr differs from CORE::substr, and cannot be used as a lvalue.
To change part of a string, you can use the optional fourth argument which is the
replacement string.

Char::Big5HKSCS::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Ebig5hkscs::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Ebig5hkscs::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Ebig5hkscs::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Ebig5hkscs::MATCH()       $1
  $MATCH          Char::Ebig5hkscs::MATCH()       $1
  ${^MATCH}       Char::Ebig5hkscs::MATCH()       $1
  $'              Char::Ebig5hkscs::POSTMATCH()   $'
  $POSTMATCH      Char::Ebig5hkscs::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Ebig5hkscs::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Ebig5hkscs::*(),
Char::Ebig5hkscs::lstat(), and Char::Ebig5hkscs::stat() on Microsoft Windows open the file for the path
...
Char::Big5HKSCS software remains one language and one interpreter.
