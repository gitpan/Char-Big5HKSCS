    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # /i modifier
                ...
                my $lc = Char::Ebig5hkscs::lc($char[$i]);
                if ($uc ne $lc) {
                    $char[$i] = '[' . $uc . $lc . ']';
                }
            }
...
sub Char::Ebig5hkscs::T(;*@) {
    ...
    local $_ = shift if @_;
    croak 'Too many arguments for -T (Char::Ebig5hkscs::T)' if @_ and not wantarray;
    my $T = 1;
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Ebig5hkscs::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Ebig5hkscs::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Char::Ebig5hkscs::split returns
...
=item Chop string
  ...
  The Char::Ebig5hkscs::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Ebig5hkscs::index($string,$substr,$position);
  $pos = Char::Ebig5hkscs::index($string,$substr);
  ...
  the Char::Ebig5hkscs::index function returns -1.

=item Reverse index string

  $pos = Char::Ebig5hkscs::rindex($string,$substr,$position);
  $pos = Char::Ebig5hkscs::rindex($string,$substr);

  Works just like Char::Ebig5hkscs::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Char::Ebig5hkscs::chr(65) is "A" in either ASCII or Big5-HKSCS, not Unicode,
  and Char::Ebig5hkscs::chr(0x82a0) is a Big5-HKSCS HIRAGANA LETTER A. For the reverse of
  Char::Ebig5hkscs::chr, use Char::Big5HKSCS::ord.

=item File test operator -X

  A file test operator is an unary operator that tests a pathname or a filehandle.
  If $string is omitted, it uses $_ by function Char::Ebig5hkscs::r_.
  The following functions function when the pathname ends with chr(0x5C) on MSWin32.

  $test = Char::Ebig5hkscs::r $string;
  $test = Char::Ebig5hkscs::r_;
  ...
  a filehandle. Both Char::Ebig5hkscs::T and Char::Ebig5hkscs::B deosn't work when given the special
  ...
  $value = Char::Ebig5hkscs::s $string;
  $value = Char::Ebig5hkscs::s_;
...
=item Statistics about file
  ...
  @stat = Char::Ebig5hkscs::stat($file);
  ...
  ($dev,$ino,$mode,$nlink,$uid,$gid,$rdev,$size,
      $atime,$mtime,$ctime,$blksize,$blocks) = Char::Ebig5hkscs::stat($file);
...
=item require file
  ...
  If the argument is a string, Char::Ebig5hkscs::require loads and executes the Perl code found in
  the separate file whose name is given by the string. This is similar to performing a
  Char::Ebig5hkscs::do on a file, except that Char::Ebig5hkscs::require checks to see whether the library
  ...
  duplicate compilation.) Like its cousins Char::Ebig5hkscs::do and use, Char::Ebig5hkscs::require knows how
