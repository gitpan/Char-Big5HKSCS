...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ebig5hkscs::uc($char[$i]) ne Ebig5hkscs::lc($char[$i]))) {
            $char[$i] = '[' . Ebig5hkscs::uc($char[$i]) . Ebig5hkscs::lc($char[$i]) . ']';
        }
...
sub e_require {
    ...
    return qq<Ebig5hkscs::require '$expr';>;
}
...
http://search.cpan.org/dist/Char-Big5HKSCS/Big5HKSCS/JA.pm
...
Definitions in Ebig5hkscs.pm.
  ...
  @{Ebig5hkscs::eV}             qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x0C\x0A\x0D])}
...
=item * Length By Big5-HKSCS Character
  ...
  $string is omitted, it returns the Big5HKSCS::length of $_.
...
=item * Substr By Big5-HKSCS Character
  ...
  area past the end of the string, Big5HKSCS::substr raises an exception.
...
=item * Rindex By Big5-HKSCS Character
  ...
  $rindex = Big5HKSCS::rindex($string,$substring,$position);
  $rindex = Big5HKSCS::rindex($string,$substring);

  This function works just like Big5HKSCS::index except that it returns the position of
